...
export function BarModeScreen({
  ...
  // Estado local para manejar qué inputs están expandidos por participante (index)
  ...
  console.log("RENDERIZANDO BarModeScreen. ID actual:", currentJuntadaId); // <-- AGREGA ESTO

 // 1. Cargar juntada cuando llega el ID por la URL (Prioridad alta)
useEffect(() => {
  if (juntadaId && juntadaId !== currentJuntadaId) {
    setCurrentJuntadaId(juntadaId);
    loadJuntada(juntadaId);
  }
}, [juntadaId, currentJuntadaId]); // Agregamos currentJuntadaId como dependencia de seguridad


  // Esto solo vive en la memoria de React, no toca la base de datos
// 1. Cargamos al usuario en memoria, pero NO lo guardamos en Supabase todavía.
// Esto evita que se cree un registro en la DB al entrar a la pantalla.
useEffect(() => {
  if (!juntadaId && !dataLoaded && participants.length === 0 && userName) {
    const usuarioInicial: Participant = {
      id: '',
      juntada_id: '',
      name: userName,
      alias_bancario: '',
    ...
    setParticipants([usuarioInicial]);
  }
}, [userName, dataLoaded]);

  const ensureJuntadaExists = async (): Promise<string | null> => {
  // Si ya tiene ID, ya existe en DB, devolvemos ese ID
  if (currentJuntadaId) return currentJuntadaId;

  // Si no, creamos la juntada y el participante inicial
  const { data: juntada, error: errJuntada } = await supabase
    .from('juntadas')
    .insert({ name: name || 'Bar', mode: 'bar', total: total })
    .select('id')
    .single();

  if (errJuntada || !juntada) return null;

  // Ahora creamos al participante (el dueño) en la base de datos
...
// 2. Inicializar (Solo carga si existe ID, no crea nada automáticamente)
useEffect(() => {
  const init = async () => {
    // 1. Si hay ID en la URL, lo cargamos.
    if (juntadaId) {
      setCurrentJuntadaId(juntadaId);
      await loadJuntada(juntadaId);
      return;
    }

    // 2. Si no hay ID, NO HACEMOS NADA. 
    // La juntada se creará solo cuando el usuario haga un cambio 
    // (al guardar un nombre, agregar participante o editar el total).
    console.log("Esperando acción del usuario para crear la juntada...");
  };
  ...
  if (currentJuntadaId && userName) {
    const registrarInvitado = async () => {
      ...
      if (!existing) {
        ...
        await loadJuntada(currentJuntadaId);
      }
    };
    registrarInvitado();
  }
}, [currentJuntadaId, userName]); // Ahora depende del ID real, no del ID de la URL

  // 3. Recalcular cuando cambian valores clave
  useEffect(() => {
    recalculate();
  }, [total, tipPercentage, participants]);

  

  const loadJuntada = async (id: string) => {
    console.log("Intentando cargar juntada con ID:", id); // <--- AGREGA ESTO
    try {
      ...
      if (juntada) {
        ...
        setTipPercentage(juntada.tip_percentage);
      }
...
const saveJuntada = async () => {
  try {
    if (currentJuntadaId) {
      // 1. SI YA EXISTE ID, ACTUALIZAMOS
      console.log("Actualizando juntada existente:", currentJuntadaId);
      await supabase
        .from('juntadas')
        .update({
          name,
          ...
          extra_expenses_enabled: true,
        })
        .eq('id', currentJuntadaId);
    } else {
      // 2. SI NO EXISTE, CREAMOS LA JUNTADA PRIMERO
      console.log("Creando nueva juntada...");
  ...
  .single();
      ...
      if (data) {
        console.log("Juntada creada con éxito, ID:", data.id);
        setCurrentJuntadaId(data.id); // Guardamos el nuevo ID
      }
    }
  } catch (error) {
    console.error('Error al guardar/crear juntada:', error);
  }
};
 ...
 const addParticipant = async () => {
    console.log("Botón presionado. currentJuntadaId es:", currentJuntadaId);
    
    // Si no hay ID, forzamos la creación antes de agregar nada
    if (!currentJuntadaId) {
        console.log("No hay ID, intentando crear juntada manualmente...");
        await saveJuntada(); // Esto debería crear la juntada si currentJuntadaId está vacío
    }

    const newParticipant: Participant = {
      id: '',
      juntada_id: currentJuntadaId || '', // Aquí debería haber un ID después de la llamada anterior
      name: '',
      alias_bancario: '',
      pago_efectivo: false,
  ...
  const setRecaudador = async (index: number) => {
    ...
    if (currentJuntadaId && recaudadorId) {
      await supabase
        .from('participants')
        .update({ is_recaudador: false, alias_bancario: '' })
        .eq('juntada_id', currentJuntadaId);

      await supabase
        .from('participants')
        .update({ is_recaudador: true, alias_bancario: participants[index].alias_bancario })
        .eq('id', recaudadorId);
    }
  };
  ...
  const copyForWhatsApp = () => {
    ...
    if (calculations.tip > 0) {
      text += `Propina (${tipPercentage}%): ${formatCurrency(calculations.tip)}\n`;
    }
    ...
    calculations.balances.forEach((b) => {
      if (b.isRecaudador) {
        ...
        const partOriginal = participants?.find(p => p.id === b.id || p.name === b.name);
          ...
          <div className="space-y-3">
            {participants.map((participant, index) => {
              ...
              return (
                ...
                >
                  {/* RENGLÓN 1 - Recaudador, Nombre, Eliminar */}
                  ...
                  {/* RENGLÓN 2 - Fila de 3 Botones Distribuidos en Grid */}
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    {/* Botón 1: CBU / Alias */}
                    <button
                      onClick={() => {
                        if (!participant.is_recaudador) {
                          alert("No puede porque no recauda");
                          return;
                        }
                  ...
                  {/* RENGLÓN 3 - Zonas Desplegables Condicionales */}
                  {((isExpanded.alias && participant.is_recaudador) || isExpanded.gasto) && (
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800/60 flex flex-col gap-2.5">
                      ...
                      {/* Input ÚNICO Estable para el Gasto Individual */}
                      {isExpanded.gasto && (
                        <div className="flex flex-col gap-1">
            ...
            {participants.find(p => p.is_recaudador)?.alias_bancario && (
              <button
                onClick={() => {
                  ...
                  alert('¡Alias copiado solo! Listo para mandar o pasar al banco.');
                }}
